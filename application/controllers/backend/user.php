...
class User extends CI_Controller {
    ...
    public function view() {
        ...
        //Template
        $this->template->backend('users', $data);
    }
    ...
    public function profile() {
        if ($this->input->post('profile')) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['title'] = 'Profile';
                $result = $this->user_model->get($this->session->userdata('user_id'));
    ...
    public function dev (){
        ...
        if ($this->input->post()) {
            $keyword = $this->input->post('keyword');
            $field = 'user_' . $this->input->post('field');
            $config['total_rows'] = $this->user_model->countdev($keyword, $field);
            $data['dev'] = $this->user_model->viewdev($keyword, $field, $value, $offset);
        } else {
            $config['total_rows'] = $this->user_model->countdev(NULL, NULL);
            $data['dev'] = $this->user_model->viewdev(NULL, NULL, $value, $offset);
        }
        $this->pagination->initialize($config); //Some config in application/config/pagination.php
        $data['pagination'] = $this->pagination->create_links();
